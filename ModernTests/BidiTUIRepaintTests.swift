...
class BidiTUIRepaintTests: XCTestCase {
    ...
    func testZshPartialLineEditRefreshesBidiMapping() {
        ...
        // Compare against a full repaint of the same final text, rather than a
        // hardcoded visual order: the invariant is that a partial in-place edit
        // draws identically to painting the final row from scratch, whatever the
        // bidi algorithm resolves that row to.
        let fullScreen = makeScreen(width: width, height: 6)
        frame(fullScreen) { mutableState in
            mutableState.appendString(atCursor: afterText)
        }
        let expectedVisual = visualOrder(fullScreen, row: 0).visual.trimmingCharacters(in: .whitespaces)
        ...
        // The cursor must map to the remaining `ou`, not to its pre-edit column.
