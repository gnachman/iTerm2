...
final class MessagesSinceResponderTests: XCTestCase {
    ...
    func testSummarize_dropsStatusOnlyMultipartAgentMessage() {
        // A status-only / reasoning agent message is not hiddenFromClient, but it
        // has no displayable substance - the same predicate the push trigger uses.
        // It must NOT be previewed (no reasoning text leaking onto the lock screen)
        // when a later, substantive message's push sweeps it into the window.
        let status = LLM.Message.Attachment(
            inline: true, id: "s1",
            type: .statusUpdate(.reasoningSummaryUpdate("thinking…")))
        let statusOnly = msg(.multipart([.attachment(status)], vectorStoreID: nil))
        let real = msg(.markdown("the answer"))
        // fetched is newest-first: the real reply, then the older status-only one.
        let r = MessagesSinceResponder.summarize(fetched: [real, statusOnly],
                                                 limit: 10, bodyMaxLength: 100)
        XCTAssertEqual(r.previews.map { $0.uniqueID }, [real.uniqueID],
                       "a status-only agent message has no substance; only the real reply previews")
    }
}
