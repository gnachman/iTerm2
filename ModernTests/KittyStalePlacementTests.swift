...
final class KittyStalePlacementTests: XCTestCase {
    ...
    // Retransmitting an id with a smaller bitmap must clamp a surviving placement's source rect to
    // the new bitmap, so it does not draw a texture frame outside the image bounds.
    func testRetransmitSmallerBitmapClampsSourceRect() {
        let controller = makeController()

        // Display image 7 (64x64) with an explicit full-size source rect and placement id 1.
        controller.execute(command: KittyImageCommand("a=T,f=100,t=d,i=7,U=1,c=4,r=2,p=1,x=0,y=0,w=64,h=64,q=2;\(redPNGBase64)")!)
        // Retransmit id 7 with a 32x32 bitmap and no display, so the placement survives and is
        // re-pointed at the smaller image.
        controller.execute(command: KittyImageCommand("a=t,f=100,t=d,i=7,q=2;\(green32PNGBase64)")!)

        guard let draw = controller.draws().first(where: { $0.imageID == 7 && $0.placementID == 1 }) else {
            XCTFail("the placement should survive a bare retransmission")
            return
        }
        let bounds = draw.image.scaledSize
        let epsilon: CGFloat = 0.5
        XCTAssertGreaterThanOrEqual(draw.sourceFrame.minX, -epsilon)
        XCTAssertGreaterThanOrEqual(draw.sourceFrame.minY, -epsilon)
        XCTAssertLessThanOrEqual(draw.sourceFrame.maxX, bounds.width + epsilon,
                                 "source rect must be clamped to the new (smaller) bitmap width")
        XCTAssertLessThanOrEqual(draw.sourceFrame.maxY, bounds.height + epsilon,
                                 "source rect must be clamped to the new (smaller) bitmap height")
    }

    // The source crop is normalized to fractions of the bitmap it was requested against, so a
    // retransmit to a smaller bitmap samples the same PROPORTIONAL region (matching kitty), not a
    // stale absolute box that would collapse to the whole image.
    func testRetransmitSmallerBitmapSamplesProportionalRegion() {
        let controller = makeController()

        // Display image 7 (64x64) cropped to its bottom-right 32x32 quadrant (fractions 0.5,0.5,0.5,0.5).
        controller.execute(command: KittyImageCommand("a=T,f=100,t=d,i=7,U=1,c=4,r=2,p=1,x=32,y=32,w=32,h=32,q=2;\(redPNGBase64)")!)
        // Retransmit id 7 with a 32x32 bitmap. The proportional region is now its bottom-right 16x16.
        controller.execute(command: KittyImageCommand("a=t,f=100,t=d,i=7,q=2;\(green32PNGBase64)")!)

        guard let draw = controller.draws().first(where: { $0.imageID == 7 && $0.placementID == 1 }) else {
            XCTFail("the placement should survive retransmission")
            return
        }
        let frame = draw.sourceFrame
        XCTAssertFalse(frame.origin.x.isNaN || frame.origin.y.isNaN ||
                       frame.size.width.isNaN || frame.size.height.isNaN,
                       "the source frame must not be NaN")
        // The sampled region is a quadrant (half in each dimension) of the new 32x32 bitmap, not the
        // whole image.
        let bounds = draw.image.scaledSize
        let epsilon: CGFloat = 0.5
        XCTAssertEqual(frame.size.width, bounds.width / 2, accuracy: epsilon,
                       "a normalized crop must sample a proportional region, not the whole bitmap")
        XCTAssertEqual(frame.size.height, bounds.height / 2, accuracy: epsilon)
    }

    // Clamping must not be cumulative: the requested source rect is stored raw and resolved against
    // the current bitmap, so shrinking then re-growing the bitmap restores the full source rect
    // rather than keeping the shrunken one.
    func testRetransmitLargerBitmapRestoresSourceRect() {
        let controller = makeController()

        // Display image 7 (64x64) with an explicit full-size source rect and placement id 1.
        controller.execute(command: KittyImageCommand("a=T,f=100,t=d,i=7,U=1,c=4,r=2,p=1,x=0,y=0,w=64,h=64,q=2;\(redPNGBase64)")!)
        // Shrink to 32x32, then grow back to 64x64, both as bare retransmissions.
        controller.execute(command: KittyImageCommand("a=t,f=100,t=d,i=7,q=2;\(green32PNGBase64)")!)
        ...
        // The source frame should again cover the whole 64x64 bitmap, not a 32x32 corner.
        let bounds = draw.image.scaledSize
        let epsilon: CGFloat = 0.5
        XCTAssertEqual(draw.sourceFrame.width, bounds.width, accuracy: epsilon,
                       "re-growing the bitmap must restore the full source rect width")
        XCTAssertEqual(draw.sourceFrame.height, bounds.height, accuracy: epsilon,
                       "re-growing the bitmap must restore the full source rect height")
    }

    // An out-of-range source crop must be rejected (EINVAL), not silently widened to the whole image.
    func testOutOfRangeCropIsRejected() {
        let controller = makeController()

        // Image 7 is 64x64; a crop whose origin is at the right edge (x=64) is out of range.
        controller.execute(command: KittyImageCommand("a=T,f=100,t=d,i=7,x=64,y=0,w=32,h=32,q=2;\(redPNGBase64)")!)

        XCTAssertFalse(controller.draws().contains { $0.imageID == 7 },
                       "an out-of-range source crop must not draw the whole image")
    }
    ...
    // A crop (x/y/w/h) with no c/r must be sized to the crop, not to the whole bitmap. The
    // transmitted s/v dimensions describe the whole bitmap, so they must not win over the crop.
    func testCropWithoutCellSizeUsesCropSize() {
        let controller = makeController()

        // Image 7 is 64x64 (s=64,v=64); display its 32x32 top-left quadrant with no c/r.
        controller.execute(command: KittyImageCommand("a=T,f=100,t=d,i=7,s=64,v=64,x=0,y=0,w=32,h=32,p=1,q=2;\(redPNGBase64)")!)

        guard let draw = controller.draws().first(where: { $0.imageID == 7 && $0.placementID == 1 }) else {
            XCTFail("the placement should exist")
            return
        }
        // The placement is sized to the 32x32 crop, not the 64x64 bitmap.
        let epsilon: CGFloat = 0.5
        XCTAssertEqual(draw.destinationFrame.size.width, 32, accuracy: epsilon,
                       "a crop without c/r must be sized to the crop, not the whole bitmap")
        XCTAssertEqual(draw.destinationFrame.size.height, 32, accuracy: epsilon)
    }

    // Retransmitting an image id must not resize an existing placement whose footprint was derived
    // (no c/r). The footprint is fixed when the placement is created (the cursor advanced past it and
    // later output was laid out around it); the new bitmap is scaled into that same rect.
    func testRetransmitDoesNotResizeDerivedFootprint() {
        let controller = makeController()

        // Non-virtual, no c/r and no s/v: the footprint is derived from the 64x64 bitmap.
        controller.execute(command: KittyImageCommand("a=T,f=100,t=d,i=7,p=1,q=2;\(redPNGBase64)")!)
        guard let before = controller.draws().first(where: { $0.imageID == 7 && $0.placementID == 1 }) else {
            XCTFail("the placement should exist")
            return
        }
        let beforeSize = before.destinationFrame.size

        // Retransmit a smaller 32x32 bitmap with no display command.
        controller.execute(command: KittyImageCommand("a=t,f=100,t=d,i=7,q=2;\(green32PNGBase64)")!)
        guard let after = controller.draws().first(where: { $0.imageID == 7 && $0.placementID == 1 }) else {
            XCTFail("the placement should survive retransmission")
            return
        }
        let epsilon: CGFloat = 0.5
        XCTAssertEqual(after.destinationFrame.size.width, beforeSize.width, accuracy: epsilon,
                       "retransmitting must not resize an existing placement's footprint")
        XCTAssertEqual(after.destinationFrame.size.height, beforeSize.height, accuracy: epsilon)
    }
