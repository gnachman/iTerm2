...
- (void)testEveryTruncationOfValidLayoutDoesNotCrash {
    ...
    for (NSUInteger i = 0; i <= valid.length; i++) {
        ...
        // Just must not throw.
        (void)[[TmuxLayoutParser sharedInstance] parsedLayoutFromString:prefix];
    }
}

// Malformed structural strings (unbalanced braces, stray delimiters) must
// return nil rather than throw from an out-of-bounds substring range.
- (void)testMalformedLayoutsReturnNilWithoutCrashing {
    ...
    for (NSString *layout in malformed) {
        // Must not throw; result may be nil or a partial tree.
        (void)[[TmuxLayoutParser sharedInstance] parsedLayoutFromString:layout];
    }
}
