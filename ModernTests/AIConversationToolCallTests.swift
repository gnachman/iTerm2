...
final class AIConversationToolCallTests: XCTestCase {
    ...
    /// Tests that DeepSeekStreamingResponseParser correctly parses multiple parallel tool_calls.
    ///
    /// This is a deterministic unit test that directly exposes the bug at DeepSeek.swift:184:
    /// ```swift
    /// if let call = choice.delta.tool_calls?.first {
    /// ```
    ///
    /// When DeepSeek sends multiple tool_calls in a single streaming chunk, only the first
    /// one is parsed. This causes "insufficient tool messages" errors because we only
    /// respond to one tool_call when DeepSeek expects responses for all of them.
    func testDeepSeekParser_MultipleParallelToolCalls_OnlyFirstIsParsed() throws {
        // This JSON simulates a DeepSeek streaming response with THREE parallel tool_calls
        // in a single chunk. This is how DeepSeek signals parallel function calls.
        ...
        // BUG: The current implementation only returns 1 message (for the first tool_call)
        // because of `.first` at DeepSeek.swift:184.
        //
        // EXPECTED: 3 messages (one for each parallel tool_call)
        // ACTUAL: 1 message (only the first tool_call)

        print("=== Parallel Tool Calls Parser Test ===")
        print("Input: 3 parallel tool_calls (apple, banana, cherry)")
        print("Output: \(messages.count) message(s)")

        for (i, msg) in messages.enumerated() {
            if let call = msg.function_call {
                print("  Message \(i): function_call id=\(call.id ?? "nil"), name=\(call.name ?? "nil")")
            }
        }

        // This assertion will FAIL, demonstrating the bug
        XCTAssertEqual(messages.count, 3,
                       "BUG: Parser only returns \(messages.count) message(s) instead of 3. " +
                       "The `.first` at DeepSeek.swift:184 drops parallel tool_calls.")

        // If the bug is fixed, also verify all tool_calls are present
        if messages.count == 3 {
            let callIDs = messages.compactMap { $0.function_call?.id }
            XCTAssertTrue(callIDs.contains("call_apple"), "Should have apple tool_call")
            XCTAssertTrue(callIDs.contains("call_banana"), "Should have banana tool_call")
            XCTAssertTrue(callIDs.contains("call_cherry"), "Should have cherry tool_call")
        }
    }
    ...
    /// Tests the scenario where DeepSeek streams tool_calls across multiple chunks.
    /// Each chunk contains one tool_call, which is a different pattern than parallel calls.
    func testDeepSeekParser_SequentialToolCallChunks_Work() throws {
        // First chunk: first tool_call
        let chunk1 = """
        {
            "id": "chatcmpl-abc123",
            "object": "chat.completion.chunk",
        ...
        // Second chunk: second tool_call (in a separate streaming event)
        let chunk2 = """
        {
            "id": "chatcmpl-abc123",
            "object": "chat.completion.chunk",
        ...
        XCTAssertEqual(messages2.count, 1, "Second chunk should have 1 tool_call")
        XCTAssertEqual(messages2.first?.function_call?.id, "call_second")

        print("=== Sequential Tool Call Chunks Test ===")
        print("This pattern WORKS because each chunk has only one tool_call.")
        print("The bug only manifests when multiple tool_calls are in the SAME chunk.")
    }
}
