...
struct ScanningView: View {
    ...
    private func handle(_ string: String, quietOnMalformed: Bool = true) {
        // Pause while the system-camera link's "Pair with this Mac?" dialog is up:
        // the live scanner would otherwise pair the same QR a second time behind
        // the dialog. Returning without setting `handled` lets scanning resume if
        // the user cancels the dialog.
        guard model.pendingExternalPairing == nil else { return }
