...
enum PhoneIdentity {
    ...
    static func migrateSharedItemsToAppGroup() {
        do {
            ...
            // companionLog (not NSLog) so this lands in the exportable Companion
            // log; a migration failure is exactly what those logs exist to
            // diagnose.
            companionLog("PhoneIdentity: keychain access-group migration failed: \(error)")
        }
    }
    ...
    private static func baseQuery(account: String, accessGroup: String?) -> [String: Any] {
        CompanionKeychainQuery.base(service: service, account: account, accessGroup: accessGroup)
    }
...
private struct SecItemStore: KeychainItemStore {
    ...
    func readAll(account: String) throws -> [KeychainItemCopy] {
        // Unscoped + all matches + attributes, so we learn every copy and the
        // concrete access group each lives in (needed to delete the source copy
        // scoped to its own group, never an unscoped delete that hits the target).
        var query = baseQuery(account: account, accessGroup: nil)
        ...
        return items.compactMap { dict in
            ...
            return KeychainItemCopy(accessGroup: dict[kSecAttrAccessGroup as String] as? String,
                                    data: data)
        }
    }
