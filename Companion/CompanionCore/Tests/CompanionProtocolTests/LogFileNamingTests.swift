...
final class LogFileNamingTests: XCTestCase {
    ...
    func test_labelWithDigitsAndHyphensStillParses() {
        // A label containing a digit or hyphen must not make the file unparseable
        // (which would leak it from pruning and the emailed export). The timestamp
        // is fixed-width, so any label after it is fine.
        let when = Date(timeIntervalSince1970: 1_781_640_000)
        for label in ["nse2", "nse-1", "app-2026", "x-y-z"] {
            let name = LogFileNaming.fileName(for: when, label: label)
            let parsed = LogFileNaming.date(from: name)
            XCTAssertEqual(parsed?.timeIntervalSince1970 ?? -1, when.timeIntervalSince1970, accuracy: 1.0,
                           "label \(label) should still yield a parseable timestamp")
        }
    }
