...
final class RelayJoinTests: XCTestCase {
    ...
    func test_transcriptLayout() {
        ...
        let expected = CanonicalEncoding.encode(
            ...
            [Data([RelayJoin.protocolVersion]), Data([RelayJoin.Role.phone.rawValue]),
             nonce, Data(roomName.utf8), Data(origin.utf8)])
    ...
    func test_deleteTranscriptLayout() {
        let t = RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin)
        let expected = CanonicalEncoding.encode(
            domain: "iterm2-relay-delete",
            [Data([RelayJoin.protocolVersion]), nonce, Data(roomName.utf8), Data(origin.utf8)])
        XCTAssertEqual(t, expected)
    }

    // A captured join signature must never authorize a deletion: the domains
    // differ, so the signed bytes can never coincide.
    func test_deleteTranscriptDomainDiffersFromJoin() {
        XCTAssertNotEqual(
            RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin),
            RelayJoin.transcript(role: .phone, nonce: nonce, roomName: roomName, origin: origin))
        XCTAssertNotEqual(
            RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin),
            RelayJoin.transcript(role: .mac, nonce: nonce, roomName: roomName, origin: origin))
    ...
    func test_deleteTranscriptVariesWithChallengeRoomOrigin() {
        var c2 = nonce; c2[0] ^= 0x01
        XCTAssertNotEqual(
            RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin),
            RelayJoin.deleteTranscript(challenge: c2, roomName: roomName, origin: origin))
        XCTAssertNotEqual(
            RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin),
            RelayJoin.deleteTranscript(challenge: nonce, roomName: "b" + roomName.dropFirst(), origin: origin))
        XCTAssertNotEqual(
            RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin),
            RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin + "x"))
    }

    func test_deleteSignAndVerifyRoundTrip() {
        let key = RelayJoin.signingKey(roomSecret: roomSecret)
        let verifier = RelayJoin.verifier(roomSecret: roomSecret)
        let t = RelayJoin.deleteTranscript(challenge: nonce, roomName: roomName, origin: origin)
        let sig = try! key.signature(for: t)
