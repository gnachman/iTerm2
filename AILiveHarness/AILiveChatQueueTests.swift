...
extension AILiveHarness {
    ...
    /// Repros the production bug captured in debuglog.txt:
    ///
    ///   line 6070: ChatBroker squelches the .remoteCommandRequest for
    ///   `execute_command` (toolu_01PLXwdeLhPsv5J49rNUK2uk) because
    ///   ChatClient.processRemoteCommandRequest returns nil from the
    ///   broker processor on the .always-permission branch.
    ///   line 7508: the corresponding .remoteCommandResponse IS persisted.
    ///   line 24724: Anthropic 400 on B's turn — "unexpected tool_use_id
    ///   found in tool_result blocks: toolu_01PLXwdeLhPsv5J49rNUK2uk.
    ///   Each tool_result block must have a corresponding tool_use block
    ///   in the previous message."
    ///
    /// Mechanism: ChatBroker.publish skips listModel.append when a
    /// processor returns nil (ChatBroker.swift:132-140), so the
    /// tool_use never lands in the chat DB. For A's own turn this is
    /// fine because conversation.messages is in-memory and already
    /// contains the function_call from the LLM response. But when B's
    /// turn is dequeued, ChatAgent.fetchCompletion calls
    /// load(messages: history) which rebuilds conversation.messages
    /// from the DB. The DB has the tool_result but no tool_use; the
    /// translation emits a functionOutput with no preceding
    /// functionCall; Anthropic rejects with 400.
    ///
    /// This test registers a broker processor that mirrors the
    /// ChatClient.processRemoteCommandRequest squelch (same return
    /// value, same async response publish), forcing the bug to
    /// reproduce without depending on a real PTYSession or ChatClient
    /// instantiation.
    ///
    /// FAILING on current main; PASSES once ChatAgent.translate
    /// synthesizes a missing tool_use whenever a tool_result references
    /// a request id that was never persisted.
    func test_chat_squelchedToolUseHistoryDoesNotBreakNextTurn() throws {
        let providerName = AITermController.provider?.displayName ?? ""
        let envKey: String?
        switch providerName {
        case "OpenAI":     envKey = Self.configValue("OPENAI_API_KEY")
        ...
        let chatID = try broker.create(
            chatWithTitle: "live squelched-tool-use test \(UUID().uuidString.prefix(8))",
            terminalSessionGuid: "queue-test",
            browserSessionGuid: nil,
            permissions: perms,
            initialMessages: [])
        ...
        // The squelcher: mirrors ChatClient.processRemoteCommandRequest's
        // .always branch — captures the request, schedules an async
        // .remoteCommandResponse, and returns nil so the request is
        // dropped from broker delivery AND from listModel.append.
        let toolDelay: TimeInterval = 1.0
        let sawToolRequest = expectation(description: "tool request seen via processor squelch")
        let squelcher = SquelchingToolProcessor(broker: broker,
                                                chatID: chatID,
                                                delay: toolDelay,
                                                output: "OpenSSL 3.6.2",
                                                onSquelch: { sawToolRequest.fulfill() })
        defer { squelcher.detach() }

        let sawATurnEnd = expectation(description: "A's turn ends (typing=false)")
        let sawBTurnEnd = expectation(description: "B's turn ends (typing=false)")
        var agentTurnEnds = 0
        recorder.onEntry = { entry, _ in
            if case let .typing(isTyping, participant) = entry,
               !isTyping, participant == .agent {
                agentTurnEnds += 1
                if agentTurnEnds == 1 { sawATurnEnd.fulfill() }
                else if agentTurnEnds == 2 { sawBTurnEnd.fulfill() }
            }
        }

        try broker.publish(message: userText(chatID: chatID,
                                             body: "Use execute_command to run any command. The output doesn't matter; just call the tool."),
                           toChatID: chatID, partial: false)
        wait(for: [sawToolRequest], timeout: 60)

        try broker.publish(message: userText(chatID: chatID,
                                             body: "Now reply with the single digit answer to 1+1, nothing else."),
                           toChatID: chatID, partial: false)
        ...
        let dump = recorder.summary(formatContent: short)
        print("[squelch-bug-test] broker log:\n\(dump)")

        // Find any agent-authored message that's an error notice. If
        // the bug is present, B's turn produces "An error ocurred:
        // Error from Anthropic: HTTP request failed with status 400…"
        // because the tool_use is missing from the rebuilt history.
        let errorIdx = recorder.firstDelivery { msg in
            guard msg.author == .agent else { return false }
            guard let text = msg.content.simpleText else { return false }
            return text.contains("HTTP request failed") || text.contains("An error ocurred")
        }
        XCTAssertNil(errorIdx,
                     "B's turn errored — squelched tool_use breaks history reconstruction. " +
                     "Expected ChatAgent.translate to synthesize a tool_use for orphan tool_result blocks.")
    }
...
/// Mirrors ChatClient.processRemoteCommandRequest's `.always` branch:
/// captures the request, schedules an async .remoteCommandResponse,
/// and squelches the request from broker delivery by returning nil.
/// Used to repro the production history-corruption bug without
/// instantiating ChatClient (which would need a real PTYSession).
@MainActor
private final class SquelchingToolProcessor {
    private weak var broker: ChatBroker?
    private let chatID: String
    private let delay: TimeInterval
    private let output: String
    private let onSquelch: () -> Void
    private var fired = false
    init(broker: ChatBroker,
         chatID: String,
         delay: TimeInterval,
         output: String,
         onSquelch: @escaping () -> Void) {
        self.broker = broker
        self.chatID = chatID
        self.delay = delay
        self.output = output
        self.onSquelch = onSquelch
        // Broker.processors has no removal API; the closure holds a
        // weak reference to self so after the test runs the processor
        // becomes a no-op rather than affecting subsequent tests.
        // NOT using `self?.process(...) ?? message` — that nil-
        // coalescing pattern would silently swallow the legitimate
        // squelch return (process intentionally returns nil to drop
        // the message), making the processor a no-op even when self
        // is alive.
        broker.processors.append { [weak self] message, msgChatID, _ in
            guard let self else { return message }
            return self.process(message, chatID: msgChatID)
        }
    }
    ...
    private func process(_ message: Message, chatID: String) -> Message? {
        guard chatID == self.chatID else { return message }
        guard !fired else { return message }
        guard case .remoteCommandRequest(let payload, _) = message.content else { return message }
        guard case .classic(let cmd) = payload else { return message }
        fired = true
        ...
        return nil  // Squelch — mirrors ChatClient.swift:161.
    }
}
