...
function Build {
  ...
  ENTITLEMENTS="$PWD/iTerm2.entitlements"
  CSID='H7V7XYVQ7D'
  codesign -o runtime --entitlements "$ENTITLEMENTS" --timestamp -s "$CSID" "build/$BUILDTYPE/iTerm2.app/Contents/Frameworks/Sparkle.framework/Versions/A/Resources/Autoupdate.app/Contents/MacOS/fileop"
  codesign -o runtime --entitlements "$ENTITLEMENTS" --timestamp -s "$CSID" "build/$BUILDTYPE/iTerm2.app/Contents/Frameworks/Sparkle.framework/Versions/A/Resources/Autoupdate.app/Contents/MacOS/Autoupdate"
  ...
  # Sadly for me this stopped working. See radar 44890538.
  # Notarize
