...
Rooms are named by an unguessable pseudonym, not the raw pid:
    ...
    roomName = sha256("iterm2-room-v1" || rs || pid)
...
4. The relay pins the registrant at first registration: the attest
   ...
   otherwise. Any later re-registration or rotation of V must prove
   possession of the CURRENT signing key (a signature over a fresh
   challenge) before the new V is accepted. Without this,
   "registration runs on every connect" would double as an overwrite
   channel for anyone who learns the room name (the only gate left
   in open mode). Rotation on unpair works because the rotating
   party still holds the current signing key.
...
wiped, app deleted), so established rooms ALSO carry an idle TTL on
the order of months. Both sides re-register/re-park on next contact,
so a long idle TTL costs live pairings nothing while bounding how
long a dead pairing's pseudonym, verifier, and key id persist in the
operator's storage.

Post-TTL squat: once the idle TTL deletes an established room, the
registrant pin is gone, so in open mode anyone holding a leaked QR
for that pairing could re-register their own verifier first and lock
the real pair out of the remote path indefinitely (LAN unaffected,
but a targeted persistent DoS from one old photograph). Two
mitigations: under ATTEST_REQUIRED the original attest key id may
reclaim the room even against a squatter's verifier; and on BOTH
sides, a join/registration rejection surfaces a "remote access needs
re-pairing" affordance rather than failing silently, so the user
always has a path out. Open-mode self-hosters accept the residual
risk, which is bounded to people who have physically photographed
their QR.

The reclaim record (the attest key id remembered past room deletion)
gets its OWN TTL, one further idle-TTL period after the room is
deleted, so key ids are not retained forever, resolving the apparent
contradiction with "the idle TTL bounds how long the key id
persists." After the reclaim window lapses, a squat simply falls
back to the re-pair affordance.
...
1. Mac: generates/loads static keypair, mints pid (16 hex chars),
   ...
   sha256("iterm2-room-v1" || rs || pid). QR content:
...
- Pairing rooms expire after a few minutes; every WS connection has
  ...
  forever. Established rooms carry a months-scale idle TTL (above).
...
- The post-deletion reclaim record (attest key id) has its own
  bounded TTL (one further idle-TTL period), so no per-pairing
  identifier is retained indefinitely.
...
- Room-relay Worker (separate deployment from PushRelay): room DO
  ...
  newest-wins among pre-auth Mac parkers, lifetimes + months-scale
  established-room idle TTL, hibernation with setWebSocketAutoResponse
  ...
  prove-current-signing-key-to-rotate, ticket-gated pairing-room
  ...
  Mac acked roomSecret), attest-key-id room reclaim post-TTL (with
  its own bounded TTL), per-room failed-auth token bucket, slow-reader
...
- Shared: PairingCode gains relay URL parsing/encoding (https-only),
  ...
  sha256("iterm2-room-v1" || rs || pid); join signing keypair from
