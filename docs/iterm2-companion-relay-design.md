...
5. **Classify the connect outcome; the class decides *where* to point next, and a shared backoff (below) decides *how long* to wait.**
   - **A re-resolve signal** (an **HTTP 421** on connect, or a **WS 4421** close on a live socket, §6.9): the host is reachable and has disowned this bucket. Re-fetch the map, or follow the `x-relay-owner` header / close-reason hint, and retry at the new owner.
