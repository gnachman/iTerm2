...
The `host` is a full relay hostname (the managed fleet names them `relay1.iterm2.com`, `relay2.iterm2.com`, ...; a self-hosted resolver may use any names), so the map is domain-agnostic and no host pattern is baked into the client. Precisely, `host` is an **authority**, not a URL: a scheme, path, userinfo, query, or fragment are all disallowed. The client always prepends `https://` (§6.10), so the transport is HTTPS/WSS by construction and the field never carries a scheme (direct mode is likewise https-only: the QR `relay=` value must be an `https` origin, `PairingCode.canonicalRelayOrigin`). The authority may be a DNS name or a **bare IP literal**, and may carry an explicit **`:port`** (omit it for the default 443). An IPv6 literal is written bracketed, exactly as it appears in a URL authority (for example `[2001:db8::1]` or `[2001:db8::1]:8443`), because the client forms `https://<host>` from it verbatim. Whatever form is chosen, that same byte string is simultaneously the box's provisioned identity, its TLS certificate name, and its origin base (§6.10, Appendix A invariant 7). The file does **not** carry `n_buckets`: it is fixed forever (Appendix A invariant 1), so the client owns the constant and the ranges simply tile `[0, N_BUCKETS - 1]`. A carried value could only ever be 65536, so it would be a second source of truth that must agree with the compiled-in constant; omitting it removes the mismatch entirely.
...
**One URL, always the latest.** The `resolver=` value is the full URL of the static shard-map JSON (e.g. `https://resolver.iterm2.com/shardmap.json`); the client GETs it verbatim, appending nothing, so the configured URL is exactly the URL fetched. It is served with a short TTL (**5 seconds**, `Cache-Control: max-age=5`; Appendix C) and replaced atomically on publish, so one fetch always returns the current map. There is deliberately **no** separate version pointer and no second round-trip: the map carries its own `version` (a monotonically increasing integer counter, never a timestamp; see §7.1), so a client or relay that reads a briefly-stale copy from a lagging CDN edge just sees an older version and ignores it (monotonicity, §6.6). A short TTL bounds staleness; atomic replacement prevents torn reads. The immutable-versioned-file trick (keep the big map cached forever at `shardmap-vN.json`, poll a tiny pointer for changes) pays off only when the blob is large; this map is `O(hosts)` ranges, a fraction of a KB, so re-fetching it on every poll costs almost nothing and is not worth a second request or the pointer/map consistency it would introduce. Nothing ever loads an *old* version (rollback is roll-forward, §6.4), so addressing a specific version buys nothing.
...
3. **In parallel, fetch the shard map** from the QR's `resolver=` URL (which points directly at the map JSON). If it names a different host for the bucket, **tear down the old connection and reconnect to the new host.**
4. **Ignore any map older than the newest version already seen** (monotonicity — see §6.6).
5. **Classify the connect outcome; the class decides *where* to point next, and a shared backoff (below) decides *how long* to wait.**
   - **A re-resolve signal** (an **HTTP 421** on connect, or a **WS 4421** close on a live socket, §6.9): the host is reachable and has disowned this bucket. **Force a map refresh** and retry at whatever host the map now names. Do not connect to a host just because the rejecting relay named it in an `x-relay-owner` header or close reason (that is a diagnostic hint only, §6.9); the map is the sole authority for the owner, which keeps a bad or looping config from steering clients to arbitrary hosts.
   - **A retry-here code** (429, 503, or 500 on connect; 1011, 1006, 1001, or 1008 on a live socket, §6.9): the host still owns the bucket but is busy, erroring, or restarting. Retry the **same** host and do **not** re-resolve; moving would be wrong.
   - **A connect failure** with no HTTP response and no WS close (DNS failure, TLS-handshake failure, connection refused, timeout, or network-unreachable): ambiguous, since the host may be a transient blip or may be gone/decommissioned. Re-fetch the map. If it now names a **different** host, that box was removed, so connect there. If it still names the **same** host, or the map cannot be fetched (fall back to the cached host, §8), presume a transient outage and retry the **same** host under backoff.

A brand-new pairing has no cached host, so step 2 is a cache miss and it just uses the freshly fetched map.

**Backoff, loop bound, and give-up.** Every failed attempt, of any class, increments one consecutive-failure counter that drives a single jittered backoff schedule (`RECONNECT_JITTER_INITIAL`, then full-jitter exponential to `RECONNECT_BACKOFF_CAP`, Appendix C); a successful connect-and-splice resets it. Keeping the counter shared across all three classes is what prevents a tight loop when the *where* keeps saying re-resolve but the connect keeps failing: a skew-window bounce (repeated 421 while the gaining host has not yet polled, §7.4) or a flapping host is spaced out by the same backoff as any other failure. Re-resolving is always safe to repeat, since monotonic versioning (§6.6) and reject-on-doubt (§6.5) make each re-resolve idempotent, so there is no harm in consulting the map on every failure (the fetch honors the map's short TTL, so a repeat within the window is a cheap cache hit, and it too backs off if the CDN is unreachable). For the give-up policy, distinguish the roles: a party that must stay available, a parked Mac or a phone showing a live view, retries **indefinitely** at the capped cadence and surfaces a "reconnecting" status after a few consecutive failures so the user sees the state without the retries stopping; a foreground first-connect for a brand-new pairing, which has no established session to preserve, may instead give up after a bounded time with an actionable error and let the user re-initiate.

**One resolved host per attempt.** A client resolves the owning host once per connect attempt and uses that single host for the WebSocket join *and* every room-scoped HTTP call of that attempt (`/attest/challenge`, `/attest`, `/register`, `/delete`), so it never registers on one host while parking or joining on another. On a **421** from any of them it re-resolves and **restarts the affected sub-flow from the beginning** rather than resending the same request to the new host: challenges, tickets, and registration tokens are host-local (§6.7) and the App Attest clientData is origin-bound (`SHA256(canonical("iterm2-relay-attest", [challenge, origin]))`, §6.10), so a value minted on the old host is not merely unknown at the new one, its origin is wrong and it would fail verification there. In practice a 421 mid-establishment just re-runs the self-healing attest/register sequence of §6.7 against the new owner. That origin binding is also the backstop: even a call that slipped through to the wrong host could not be accepted, because its challenge and assertion are bound to that host's origin.
...
- **Newly admit a connection only if the host's own copy of the map says it owns that bucket; otherwise reject** ("reject-on-doubt"), signaling the client to re-resolve with **HTTP 421** (§6.9). If the host knows the current owner it may add an `x-relay-owner` header naming it, but only as a diagnostic hint: the client re-resolves via the map rather than trusting the header (§6.9), so this is not an authoritative redirect and does not save the map round-trip.
- **The room-scoped HTTP endpoints reject-on-doubt on the same rule.** `/attest/challenge`, `/attest`, `/register`, and `/delete` all carry the room header, so a host applies the identical ownership gate and returns **HTTP 421** (§6.9) when it does not own the bucket. Check ownership **early** for these, before reading the body, running the App Attest verification, or charging the `/attest` rate limiter, so a stale-map bounce is neither counted as an attestation failure nor allowed to burn the client's limiter budget. Unlike a live splice these calls are one-shot and hold no ongoing connection, so there is **no drain defer for them**: the moment a host sees it has lost a bucket it rejects that bucket's HTTP calls (the §7.4 defer keeps only the already-established *splice* alive, never the data-plane calls). Accepting one on a non-owner would merely strand host-local state (a challenge, a ticket, a registration token, or a verifier) that the true owner cannot see, dead-ending the pairing. `/delete` stays best-effort under this rule: during a move it may reach the new owner, which has no such room, while the live splice finishes draining on the old owner; the drain and the client closing its own sockets both reclaim the room, so teardown still completes and it is deliberately not special-cased to chase the live-room host.
...
- **Reconnect-auth** (`verifier`, `registrantKeyId`, assertion counter). The only durable-looking state, and it is **soft state**. The join verifier is deterministic: `verifier = HKDF(roomSecret)` (`RelayJoin`), and `roomSecret` is persisted on **both** devices (the phone mints it and keeps it; it is couriered to the Mac over the Noise channel and kept there). Both roles sign their joins with that one shared key (over the `(protocolVersion, role, nonce, roomName, origin)` transcript of §6.10, not `roomName` alone), and the relay stores only the public verifier. So a host that lacks the verifier just treats the room as unestablished, and the phone re-registers the identical value.

**Switching relays means re-establishing on the new one.** Worth stating plainly, because it is the client's entire obligation on a reshard: whenever a client is moved to a different relay (a reshard, a host restart, or a host death), the destination is almost always a box that has never held this room, so it holds no verifier and the room must be established there afresh. That re-establishment is **silent** (no QR, no SAS, no user prompt) and **asymmetric** between the roles: the **phone** re-runs the attest-and-register dance, while the **Mac** only re-parks and needs no action. The phone does not re-attest speculatively; it tries its normal signed join first and falls into the dance only when the new host answers `"unestablished"`. Because reshards are rare and operator-initiated, this cost (including an occasional fresh App Attest key) is paid at reshard cadence, not routinely.
...
- **Poll cadence.** Each relay re-fetches the short-TTL `shardmap.json` every `SHARDMAP_POLL_INTERVAL` (default **10 seconds**; a few seconds to a minute, operator's choice; Appendix C); when the fetched map's `version` exceeds the one it holds, it applies the §6.5 diff (an equal-or-older version, e.g. from a lagging edge, is ignored).
- **Monotonic + integrity, same as clients (§6.6, §9).** A relay polling a multi-edge CDN can read an older version from a lagging edge, so it ignores any version older than the newest it has seen. It fetches over HTTPS from our domain; map signing is not required today (§9), but if it is ever added the relay verifies the signature before adopting a map, since it enforces ownership from this file.
...
**The client never connects to a host just because a relay named it.** On a re-resolve it derives the target from the shard map itself (§6.4), so a relay can only ever say "not me," never dictate where to go; the map is the sole authority and wins on any disagreement. This is deliberate hardening: trusting a relay's redirect would let one bad or malicious config bounce clients in an endless loop (X says go to Y, Y says go to X), whereas taking every target from the map means clients only ever reach map-sanctioned hosts, and any residual loop (e.g. a lagging CDN edge still naming the evicting host) is spaced out and bounded by the shared reconnect backoff (§6.4). A host in the two-phase "no owner" state, or any host that omits the hint, sends the bare form; the client behaves identically either way, since it re-fetches regardless. The trade-off is that the hint no longer saves the map round-trip it was once meant to, but that round-trip is cheap (a fraction-of-a-KB file, usually a cache hit within its short TTL), and the robustness is worth more.
...
Three of the relay's checks bind an **origin** into the bytes they verify, and the design has so far been silent on it. All three feed the fields through the same domain-separated, length-prefixed `canonicalEncode(domain, fields)` of §6.2 (a leading domain string, then each field prefixed by its 4-byte big-endian length), never a bare concatenation, so a field boundary can never be shifted to forge a colliding input, and each `domain` keeps one signed message from ever being replayed as another. Every transcript's first field is a 1-byte `protocolVersion` (currently `1`), so a signature or attestation is valid only for the protocol version that produced it:

- The **admission (join) transcript** both roles sign is `canonical("iterm2-relay-join", [protocolVersion, role, nonce, roomName, origin])` (`RelayJoin`), not `roomName` alone.
- The **App Attest clientData** the phone attests and asserts over is `SHA256(canonical("iterm2-relay-attest", [challenge, origin]))` (the challenge is the relay's nonce; the domain separation makes the challenge/origin boundary unambiguous).
- The **delete-room transcript** that authorizes an unpair is `canonical("iterm2-relay-delete", [protocolVersion, challenge, roomName, origin])`.
...
**`version` is a monotonic integer counter, not a timestamp.** Each publish increments it by at least one; it never repeats and never moves backward. A counter rather than a wall-clock time because monotonicity (§6.6) needs a total order that cannot regress on clock skew and cannot collide when two edits land close together, and because rollback is roll-forward (§6.4): the only valid successor to `vN` is a higher number, which a counter makes unambiguous. Every actor compares versions numerically and adopts only strictly-greater ones (the wire type is a non-negative integer; `ShardMap` rejects a negative version).

**Publishing is owned by a single pipeline (a to-be-built tool).** A web-based admin tool will be the sole writer of `shardmap.json`: it holds the authoritative counter, increments it on each change, and replaces the file **atomically** (write-new-then-rename, or an atomic CDN put) so no reader ever sees a torn map and no two publishes can select the same or a lower version. Concentrating writes in one tool is what makes the atomic-publish and monotonic-counter guarantees hold in practice. Hand-editing the file out of band would risk a repeated or non-increasing version (which every actor would then ignore, silently stranding the change) and must be avoided; the tool is the enforcement point for both invariants.
...
- **Jittered client reconnect backoff** — spread the handshake load over time. Use **full jitter** (a uniform random delay, the flattest scheme for a synchronized herd; equal/decorrelated jitter re-clumps it). The first reconnect after an eviction (4421), a reject (421), or a dropped socket waits a uniform `[0, RECONNECT_JITTER_INITIAL]` (default **3 s**); each subsequent failure backs off with full-jitter exponential, `delay = uniform(0, min(RECONNECT_BACKOFF_CAP, RECONNECT_BACKOFF_BASE · 2^n))` (defaults **1 s** base, **30 s** cap, `n` = failed attempts so far). The 3-second initial window keeps the user-visible glitch short (§7.2) while flattening a whole-host reconnect to a manageable arrival rate. The herd only forms on an *ungraceful* host death; a graceful reshard is already paced by the eviction rate below, so jitter is mostly the death-case backstop. See Appendix C.
...
- **A lone parked Mac widens the phone's reconnect gap to about the drain defer.** For a room with both endpoints live, the atomic per-room eviction migrates the splice cleanly (one reconnect glitch, §7.6). But a pairing whose Mac is *parked with no phone connected* is treated as an existing connection and is not evicted from the losing host until the `RESHARD_DRAIN_DELAY` + eviction-queue delay elapses, so it does not re-park on the new owner until then. A phone that connects during that window resolves (via the fresh map) to the new owner, finds no Mac parked there yet, and gets "mac offline", which is a retry-here (§6.9), not a re-resolve, so it simply retries at the capped backoff until the Mac drains and re-parks. The pairing still converges and stays safe, but the user-visible reconnect latency in this specific case is on the order of the drain defer (about 20 s at the default poll), not the few-second splice-migration glitch. Left as-is deliberately: reshards are rare and operator-initiated (§7.4), and evicting lone parks eagerly to shrink this window would add a special-case path for a rare, non-urgent event.
...
- **Serve the shard map over HTTPS** from a domain we control, so it can't be MITM'd to redirect clients to a rogue host. The resolver URL itself arrives inside the QR, so a valid QR already anchors trust in the right resolver. **Signing the JSON is not required, and is not done today.** HTTPS covers tampering in transit, and E2EE/Noise bounds the blast radius of a bad map to DoS plus metadata exposure (never content, per the points above), so a signature would only guard the narrow case of the CDN/origin itself being compromised. It can be **added later** if wanted, most plausibly if the map is ever fronted by a third-party edge that terminates our TLS (which would put that operator in the trust path); see §11.
...
- **A relay can reject, but never redirect.** A client takes its connect target only from the shard map, never from a host's claim about who the owner is (the `x-relay-owner` hint is diagnostic only, §6.9). So a compromised or misconfigured relay cannot steer clients to a host of its choosing or loop them between hosts; the worst it can do is refuse, after which the client re-resolves against the map and any retry is bounded by the shared backoff (§6.4).
...
- **Sign the shard map.** Not required today (§9): HTTPS from our domain plus the QR-anchored resolver already authenticate the map, and E2EE bounds a tampered map to DoS/metadata, not content. Adding a detached signature (public key pinned in both clients and relays, verified before adopt) would remove a TLS-terminating third-party CDN from the trust path. Revisit only in that configuration; it adds key management and a new signature-mismatch availability mode, so it is net-negative until then.
...
## Appendix C: Tunable parameters and default values

Starting values for the managed fleet. All are operator-tunable; the **relationships** between them (not the absolute numbers) are what the correctness arguments in §6-§7 depend on.

| Parameter | Default | Role |
|---|---|---|
| `SHARDMAP_TTL` (CDN `Cache-Control: max-age`) | **5 s** | Bounds how stale a CDN edge copy can be. Kept below the poll interval so a poll usually revalidates to a fresh copy (§6.3). |
| `SHARDMAP_POLL_INTERVAL` | **10 s** | How often each relay (and a client's background refresh) re-fetches `shardmap.json`. Sets how fast a reshard lands (§6.8). |
| `RESHARD_DRAIN_DELAY` | **20 s** (= 2 × poll) | How long a relinquishing host waits after seeing the new map before it starts evicting, covering worst-case poll-phase skew between two hosts plus TTL/propagation margin (§7.4). |
| `RESHARD_EVICTION_RATE` | **10 rooms/s** | Rate a relinquishing host closes live rooms, capping the reconnect/handshake load a move imposes on the new owners (§7.4). |
| `RECONNECT_JITTER_INITIAL` | uniform **[0, 3 s]** | First reconnect delay after a 4421 evict, a 421 reject, or a dropped socket; full jitter, flattens a whole-host reconnect storm (§7.3). |
| `RECONNECT_BACKOFF_BASE` / `RECONNECT_BACKOFF_CAP` | **1 s** / **30 s** | Full-jitter exponential backoff on repeated failure: `delay = uniform(0, min(CAP, BASE · 2^n))`, `n` = failed attempts (§7.3). |

**Relationships that must hold** (the numbers may change together, but these orderings must not break):

- **TTL < poll interval.** A poll should usually see a fresh edge copy (5 s < 10 s).
- **`RESHARD_DRAIN_DELAY` ≥ 2 × poll interval.** One interval covers the worst-case poll-phase skew between the gaining and losing hosts reacting to the same publish; the second is margin for TTL, CDN propagation, and poll jitter (§7.4). This is the one relationship the drain-safety argument depends on, so it is expressed as a multiple of the poll interval, not a fixed 20 s.
- **Convergence time of a bucket move ≈ `RESHARD_DRAIN_DELAY` + rooms / `RESHARD_EVICTION_RATE`.** At the defaults, a ~300-room host drains in roughly 20 s + 30 s ≈ 50 s. Reshards are not urgent (§7.4), so this latency is a feature (it paces handshakes), not a cost.
- **Eviction rate paces the graceful case; jitter paces the ungraceful case.** A planned reshard's reconnect load is bounded by `RESHARD_EVICTION_RATE`; only an unplanned host death drops every room at once, and there the reconnect jitter (§7.3) is what flattens the storm.
