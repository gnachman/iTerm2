...
**One resolved host per attempt.** A client resolves the owning host once per connect attempt and uses that single host for the WebSocket join *and* every room-scoped HTTP call of that attempt (`/attest/challenge`, `/attest`, `/register`, `/delete`), so it never registers on one host while parking or joining on another. On a **421** from any of them it re-resolves and **restarts the affected sub-flow from the beginning** rather than resending the same request to the new host: challenges, tickets, and registration tokens are host-local (§6.7) and the App Attest clientData is origin-bound (`SHA256(challenge ‖ origin)`, §6.10), so a value minted on the old host is not merely unknown at the new one, its origin is wrong and it would fail verification there. In practice a 421 mid-establishment just re-runs the self-healing attest/register sequence of §6.7 against the new owner. That origin binding is also the backstop: even a call that slipped through to the wrong host could not be accepted, because its challenge and assertion are bound to that host's origin.
