...
Development:
	...
	xcodebuild -scheme iTerm2 -configuration Development -derivedDataPath build/Development/DerivedData -destination 'platform=macOS' -skipPackagePluginValidation ARCHS=arm64 ONLY_ACTIVE_ARCH=YES $(CODESIGN_FLAGS) $(SWIFT_PARALLEL) $(JOBS) $(XCFLAGS) && \
	chmod -R go+rX build/Development

Dep:
	xcodebuild -scheme iTerm2 -configuration Deployment -derivedDataPath build/Deployment/DerivedData -destination 'platform=macOS' -skipPackagePluginValidation ARCHS=arm64 ONLY_ACTIVE_ARCH=YES

Beta:
	cp plists/beta-iTerm2.plist plists/iTerm2.plist
	xcodebuild -scheme iTerm2 -configuration Beta -derivedDataPath build/Beta/DerivedData -destination 'platform=macOS' -skipPackagePluginValidation ENABLE_ADDRESS_SANITIZER=NO ARCHS=arm64 ONLY_ACTIVE_ARCH=YES $(CODESIGN_FLAGS) $(SWIFT_PARALLEL) $(JOBS) && \
	chmod -R go+rX build/Beta

Deployment:
	time xcodebuild -scheme iTerm2 -configuration Deployment -derivedDataPath build/Deployment/DerivedData -destination 'platform=macOS' -skipPackagePluginValidation ENABLE_ADDRESS_SANITIZER=NO ARCHS=arm64 ONLY_ACTIVE_ARCH=YES $(CODESIGN_FLAGS) $(SWIFT_PARALLEL) $(JOBS) $(XCFLAGS)

Nightly: force
	cp plists/nightly-iTerm2.plist plists/iTerm2.plist
	xcodebuild -scheme iTerm2 -configuration Nightly -derivedDataPath build/Nightly/DerivedData -destination 'platform=macOS' -skipPackagePluginValidation ENABLE_ADDRESS_SANITIZER=NO ARCHS=arm64 ONLY_ACTIVE_ARCH=YES $(CODESIGN_FLAGS) $(SWIFT_PARALLEL) $(JOBS)
...
clean:
	...
	if [ -L ThirdParty ]; then rm ThirdParty; else rm -rf ThirdParty; fi && git checkout ThirdParty
