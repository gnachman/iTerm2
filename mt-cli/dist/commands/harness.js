...
exports.harnessCommand = harnessCommand;
const inquirer_1 = __importDefault(require("inquirer"));
const ora_1 = __importDefault(require("ora"));
const fs = __importStar(require("fs-extra"));
const path = __importStar(require("path"));
const logger_js_1 = require("../utils/logger.js");
function harnessCommand(program) {
    program
        ...
        logger_js_1.logger.header('Harness Engineering Setup');
        ...
        if (await fs.pathExists(harnessDocPath)) {
            logger_js_1.logger.warn('Harness config already exists at docs/harness-engineering.md');
        ...
        const spinner = (0, ora_1.default)('Generating harness files…').start();
        ...
        logger_js_1.logger.blank();
        logger_js_1.logger.section('Generated files', [
        ...
        logger_js_1.logger.blank();
        logger_js_1.logger.info('Install hooks: cp .hooks/pre-commit .git/hooks/ && cp .hooks/pre-push .git/hooks/');
    });
}
async function runHarnessInterview(defaultName) {
    logger_js_1.logger.info('Answer a few questions to configure your Harness Engineering environment.\n');
...
function defaultHarnessConfig(name) {
    return {
        projectName: name,
...
function generatePrePushHook(c) {
    const checks = [];
    if (c.staticAnalysis.lint)
        checks.push('npm run lint 2>/dev/null || (echo "❌ Lint failed. Fix errors before pushing." && exit 1)');
    if (c.staticAnalysis.typeCheck)
        checks.push('npm run type-check 2>/dev/null || npx tsc --noEmit 2>/dev/null || true');
    if (c.staticAnalysis.formatting)
        checks.push('npx prettier --check . 2>/dev/null || (echo "⚠️  Formatting issues found." && true)');
...
function generatePreCommitHook(c) {
    return `#!/usr/bin/env bash
