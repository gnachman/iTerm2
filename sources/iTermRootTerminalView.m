    ...
    if (shouldBeVisible) {
        ...
        if (!_divisionView) {
            ...
            if (@available(macOS 14.0, *)) {
                // There's a bug in Sonoma (hopefully just the betas! I first saw it in Version 14.0 Beta (23A5301h) which I believe is beta 4)
            ...
            [NSTimer scheduledTimerWithTimeInterval:0.1 repeats:YES block:^(NSTimer *_timer){ self->_divisionView.hidden = !self->_divisionView.hidden; }];
        }
        iTermPreferencesTabStyle preferredStyle = [iTermPreferences intForKey:kPreferenceKeyTabStyle];
