...
- (void)finishInitializationWithSmartLayout:(BOOL)smartLayout
    ...
    // Gated in the handler because this notification fires for every advanced
    // setting and carries no key.
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)advancedSettingsDidChange:(NSNotification *)notification {
    // iTermAdvancedSettingsDidChange fires for ANY advanced setting change and
    // carries no key, so filter to the ones we care about: only refresh when a
    // setting actually changed the tab bar's desired height (two-row tab bar,
    // default/compact tab bar height). Unrelated settings must not rebuild the
    // window chrome of every open window.
    if (fabs([self desiredTabBarHeight] - _contentView.tabBarControl.height) < 0.5) {
        return;
    }
    [self refreshTerminal:notification];
}
...
- (CGFloat)_desiredTabBarHeight {
    ...
    if (rows > 1) {
        // The two-row height model lives in PSMTabBarControl (single source of
        // truth); pass the single-row base and let it apply the shared geometry.
        return [_contentView.tabBarControl twoRowHeightForSingleRowHeight:base];
    }
    return base;
}
...
- (void)updateTabBarStyle {
    ...
    // Assign insets before computing the height: the two-row desiredTabBarHeight
    // derives per-row content from the control's insets, so a stale/zero inset here
    // (e.g. on a theme switch) would size the bar wrong until the next layout.
