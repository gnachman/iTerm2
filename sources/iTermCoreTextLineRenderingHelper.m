...
- (instancetype)initWithLine:(CTLineRef)line string:(nonnull NSString *)string {
    self = [super init];
    if (self) {
        _line = line;
        CFRetain(_line);
        ...
        [self alignGlyphsToGridWithGlyphIndex:glyphIndexToCharacterIndex
                        ...
                        xOriginsForCharacters:xOriginsForCharacters
                                  alignToZero:alignToZero
                                    positions:positions
                         baseCharacterIndexes:[self baseCharacterIndexes]];
...
- (void)alignGlyphsToGridWithGlyphIndex:(const CFIndex *)glyphIndexToCharacterIndex
    ...
    for (CFIndex i = 0; i < length; i++) {
        ...
        const BOOL isBase = [baseCharacterIndexes containsIndex:characterIndex];
