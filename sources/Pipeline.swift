...
class PipelineQueue<T> {
    ...
    func append(_ pipeline: Pipeline<T>) {
        ...
        if self.pipelines.count == 1 {
            print("pipeline queue got a first element so begin executing it")
            pipeline.begin()
        }
    }

    private func pipelineDidComplete() {
        print("Pipeline completed. Run next if available")
...
class Pipeline<T>: CustomDebugStringConvertible {
    ...
    private func moveBlockedToEligible() {
        ...
        print("These pipeline actions are now eligible: \(newlyEligibleActions.map(\.description).joined(separator: ", "))")
    }
    ...
    private func moveEligibleToRunning() {
        while (!eligibleActions.isEmpty &&
            ...
            print("Run pipeline action \(actionToRun.description)")
            actionToRun.closure(values) { [weak self] result in
                dispatchPrecondition(condition: .onQueue(.main))
                switch result {
                case .success(let value):
                    print("Pipeline action \(actionToRun.description) completed successfuly with value \(value)")
                    self?.actionDidComplete(actionToRun.id, value: value)
                case .failure(let error):
                    print("Pipeline action \(actionToRun.description) failed with error \(error.localizedDescription)")
    ...
    private func finish(_ disposition: Disposition) {
        ...
        print("pipeline finished with disposition \(disposition)")
