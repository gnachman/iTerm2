...
class NerdFontInstaller {
    ...
    private var state = State.ground {
        didSet {
            NSLog("State became \(state.debugDescription)")
            switch state {
            case .ground:
                break
            case .downloading:
    ...
    static func start(window: NSWindow?, completion: @escaping (Error?) -> ()) {
        Self.instance = NerdFontInstaller(window, completion: completion)
    }

    private init(_ window: NSWindow?, completion: @escaping (Error?) -> ()) {
        self.window = window
    ...
    private func downloadDidComplete(location: URL?, response: URLResponse?, error: Error?) {
        ...
        if let location {
            ...
            do {
                NSLog("Move \(location.path) to \(zip.path)")
    ...
    private func install(from tempDir: String) {
        ...
        let descriptorsToInstall = neededFontPostscriptNames.compactMap { neededPostscriptName in
            return fontDescriptors.first(where: { $0.postscriptName == neededPostscriptName })
        }
        if descriptorsToInstall.count < neededFontPostscriptNames.count {
            state = .failed(.missingRequiredFonts)
            return
        }
        install(descriptors: descriptorsToInstall) { [weak self] error in
            if let error {
