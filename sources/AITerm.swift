...
class AITermController {
    ...
    private func handle(event: Event, legacy: Bool) {
        NSLog("%@", "handle(\(event)) in state \(state)")
        switch state {
        case .ground:
            NSLog("%@", "Ignore \(event) in ground state.")
            break
        ...
        case .initialized(query: let query):
            ...
            case .error(reason: let reason):
                ...
                state = .ground
            case .apiResponse:
                NSLog("%@", "Unexpected event \(event) in \(state)")
                state = .ground
            }
...
struct TokenLimitedStringBuilder {
    let charsPerToken = 4
    ...
    private(set) var approximateTokenCount = 0

    mutating func append(string: String, deletable: Bool = true) {
        let tokens = string.utf8.count / charsPerToken + 1  // +1 for separator when joined
        chunks.append(Chunk(deletable: deletable, value: string, tokens: tokens))
        approximateTokenCount += tokens
    }

    mutating func appendToLastChunk(string: String, separator: String) {
        guard let lastChunk = removeLastChunk() else {
            append(string: string)
            return
        }
        append(string: lastChunk.value + separator + string, deletable: lastChunk.deletable)
    }

    private mutating func removeLastChunk() -> Chunk? {
        guard let lastChunk = chunks.last else {
            return nil
        }
        approximateTokenCount -= lastChunk.tokens
        return chunks.removeLast()
    }

    mutating func buildChunk(separator: String, deletable: Bool = true, closure: ((String) -> ()) -> ()) {
        var lines = [String]()
        closure { piece in
            lines.append(piece)
        }
        append(string: lines.joined(separator: separator), deletable: deletable)
    }

    func joined(separator: String, maxTokens: Int) -> String? {
        var indexesToKeep = IndexSet(0..<chunks.count)
        var tokens = approximateTokenCount
        var i = 0
        while tokens > maxTokens && i < chunks.count {
            guard chunks[i].deletable else {
                i += 1
                continue
            }
            tokens -= chunks[i].tokens
            indexesToKeep.remove(i)
            i += 1
        }
        if tokens > maxTokens {
            return nil
        }
        return chunks.enumerated().filter {
            indexesToKeep.contains($0.offset)
        }.map {
            $0.element.value
