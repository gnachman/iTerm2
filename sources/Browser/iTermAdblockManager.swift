...
@objc(iTermAdblockManager)
class iTermAdblockManager: NSObject {
    
    // MARK: - Singleton
    
    @objc static let shared = iTermAdblockManager()
    
    // MARK: - Notifications
    
    @objc static let didUpdateRulesNotification = NSNotification.Name("iTermAdblockManagerDidUpdateRules")
    @objc static let didFailWithErrorNotification = NSNotification.Name("iTermAdblockManagerDidFailWithError")
    ...
    private func downloadAndUpdateRules() {
        ...
        guard let url = URL(string: urlString) else {
            let error = NSError(domain: "iTermAdblockManager", code: 1, userInfo: [
        ...
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            DispatchQueue.main.async {
                ...
                guard let data = data, let content = String(data: data, encoding: .utf8) else {
                    let error = NSError(domain: "iTermAdblockManager", code: 2, userInfo: [
...
//        low = (low + high) / 2
    ...
    private func checkForProlongedFailure() {
        ...
        if daysSinceUpdate >= maxFailureDays {
            let error = NSError(domain: "iTermAdblockManager", code: 4, userInfo: [
