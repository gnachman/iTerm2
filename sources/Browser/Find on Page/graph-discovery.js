...
(function() {
    ...
    console.log('[IFD:INIT] iframe Graph Discovery script starting...');
    ...
    if (window[DISCOVERY_SYMBOL]) {
        console.log('[IFD:INIT] Already initialized in this frame, skipping');
        return;
    }
    ...
    if (window.webkit && window.webkit.messageHandlers) {
        console.log('[IFD:INIT] WebKit message handlers available');
    } else {
        console.log('[IFD:INIT] WebKit message handlers not available - using fallback');
    }
    ...
    class FrameDiscovery {
        ...
        discoverChildren(callback) {
            ...
            if (iframes.length === 0) {
                ...
                if (callback) {
                    ...
                    // No callback - we're the main frame, handle completion directly
                    this.checkDiscoveryComplete();
                }
                return;
            }
        ...
        sendToNative(graph) {
            ...
            if (window.webkit && window.webkit.messageHandlers && 
                ...
                console.log('[IFD:RESULT] iframe Graph Discovery Complete:', graph);
    ...
    console.log('[IFD:INIT] iframe Graph Discovery script initialized successfully');
