...
@objc protocol iTermBookmarkTagEditorDelegate: AnyObject {
    func bookmarkTagEditorDidDeleteBookmark(_ controller: iTermBookmarkTagEditorWindowController, url: String)
    func bookmarkTagEditorDidUpdateTags(_ controller: iTermBookmarkTagEditorWindowController, url: String)
    func bookmarkTagEditorShouldClose(_ controller: iTermBookmarkTagEditorWindowController)
}
...
class iTermBookmarkTagEditorWindowController: NSWindowController {
    ...
    @objc private func saveChanges() {
        ...
        Task {
            ...
            await MainActor.run {
                self.delegate?.bookmarkTagEditorDidUpdateTags(self, url: self.bookmarkURL)
                self.close()
            }
        }
    }
    ...
    private func updateBookmarkTags(newTags: Set<String>) async {
        ...
        for tag in currentTags.subtracting(newTags) {
            await database.removeTagFromBookmark(url: bookmarkURL, tag: tag)
        }
        
        // Add new tags
        for tag in newTags.subtracting(currentTags) {
            await database.addTagToBookmark(url: bookmarkURL, tag: tag)
        }
    ...
    private func performDeleteBookmark() async {
        ...
        await MainActor.run {
            if success {
                self.delegate?.bookmarkTagEditorDidDeleteBookmark(self, url: self.bookmarkURL)
    ...
    @objc private func cancel() {
        delegate?.bookmarkTagEditorShouldClose(self)
        close()
    }
...
extension iTermBookmarkTagEditorWindowController: NSTokenFieldDelegate {
    func tokenField(_ tokenField: NSTokenField, completionsForSubstring substring: String, indexOfToken tokenIndex: Int, indexOfSelectedItem selectedIndex: UnsafeMutablePointer<Int>?) -> [Any]? {
        let filteredTags = allTags.filter { tag in
            tag.localizedCaseInsensitiveContains(substring)
        }
        return filteredTags
    }
