...
class iTermBrowserPointerActionPerformer: NSObject, PointerControllerDelegate {
    ...
    func sendEscapeSequence(_ text: String!, with event: NSEvent!) {
    }

    func sendHexCode(_ codes: String!, with event: NSEvent!) {
    ...
    func sendText(_ text: String!, with event: NSEvent!, escaping: iTermSendTextEscaping) {
    ...
    func newWindow(withProfile guid: String!, with event: NSEvent!) {
        delegate?.actionPerformingCurrentTerminal()?.newWindow(withBookmarkGuid: guid)
    }

    func newTab(withProfile guid: String!, with event: NSEvent!) {
        delegate?.actionPerformingCurrentTerminal()?.newTab(withBookmarkGuid: guid)
    }

    func newVerticalSplit(withProfile guid: String!, with event: NSEvent!) {
        delegate?.actionPerformingSplit(vertically: true, guid: guid)
    }

    func newHorizontalSplit(withProfile guid: String!, with event: NSEvent!) {
    ...
    func advancedPaste(withConfiguration configuration: String!, fromSelection: Bool, with event: NSEvent!) {
        // TODO
    }
