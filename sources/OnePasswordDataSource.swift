...
class OnePasswordDataSource: CommandLinePasswordDataSource {
    ...
    override var autogeneratedPasswordsOnly: Bool {
        return true
    }
    ...
    private static func standardEnvironment(token: OnePasswordTokenRequester.Auth) -> [String: String] {
        var result = ["HOME": NSHomeDirectory()]
        switch token {
        case .biometric:
            break
    ...
    private static let pathToCLI = "/usr/local/bin/op"
        ...
        init(_ args: [String],
            ...
            OnePasswordDynamicCommandRecipe<Inputs, Outputs>(
                ...
                inputTransformer: { _, token in
                    return Command(command: OnePasswordDataSource.pathToCLI,
                                   args: args,
                                   env: OnePasswordDataSource.standardEnvironment(token: token),
                                   stdin: nil)
                },
    ...
    override func checkAvailability() -> Bool {
        let maybeData = try? Command(command: Self.pathToCLI,
                                     args: ["-v"],
                                     env: [:],
                                     stdin: nil).exec().stdout
        if let data = maybeData, let string = String(data: data, encoding: .utf8), string.hasPrefix("2.") {
            return true
        }
    ...
    private func showUnavailableMessage() {
        ...
        alert.informativeText = "Version 2 of the OnePassword CLI could not be found. Check that \(OnePasswordDataSource.pathToCLI) is installed and has version 2.x."
