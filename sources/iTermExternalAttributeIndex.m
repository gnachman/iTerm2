...
- (NSData *)data {
    ...
    if (_urlCode) {  // If urlcode is unset, encode as v1 to save a bit of space.
        [encoder encodeUnsignedInt:_urlCode];
    }
    // When v3 gets introduced, remember to encode _urlCode even if it's zero when subsequent
    // values are nonzero.
    return encoder.data;
}
