...
iTermTextVertexShader(uint vertexID [[ vertex_id ]],
    ...
    out.underlineColor = static_cast<float4>(perInstanceUniforms[iid].underlineColor);
    out.alphaVector = iTermAlphaVectorForTextColor(static_cast<float4>(out.textColor));
    return out;
}
...
iTermTextVertexShaderMonochrome(uint vertexID [[ vertex_id ]],
    ...
    out.textureCoordinate = vertexArray[vertexID].textureCoordinate + perInstanceUniforms[iid].textureOffset;
    out.textColor = static_cast<float4>(perInstanceUniforms[iid].textColor);
    out.alphaVector = iTermAlphaVectorForTextColor(static_cast<float4>(out.textColor));
...
iTermTextFragmentShaderWithBlendingUnderlined(iTermTextVertexFunctionOutput in [[stage_in]],
    ...
    float4 textColor = RemapColor(in.textColor * 17.0, backgroundColor, static_cast<float4>(bwColor), colorModels);
...
iTermTextFragmentShaderMonochromeUnderlined(iTermTextVertexFunctionOutput in [[stage_in]],
    ...
    float4 textureColor = static_cast<float4>(texture.sample(textureSampler, in.textureCoordinate));
