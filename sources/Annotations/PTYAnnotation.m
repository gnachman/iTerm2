...
- (instancetype)initWithDictionary:(NSDictionary *)dict {
    ...
    if (self) {
        ...
        // Keep the UUID generated by [self init] if dictionary doesn't have one (old format)
        NSString *savedID = dict[PTYAnnotationDictionaryKeyUniqueID];
        if (savedID.length > 0) {
            _uniqueID = [savedID copy];
        }
    }
    return self;
}
