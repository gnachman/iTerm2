...
final class CompanionPresenceController: NSObject {
    ...
    @objc func start() {
        ...
        connectToastShown = isInteractivelyPresent
    ...
    /// menu bar.
    private var isInteractivelyPresent: Bool {
        let paired = controller.hasPairedDevice && iTermAdvancedSettingsModel.companionPairingAllowed()
        return paired && controller.connectionPresence == .interactive
    }
    ...
    private func refresh(animated: Bool) {
        ...
        let paired = controller.hasPairedDevice && iTermAdvancedSettingsModel.companionPairingAllowed()
        let present = isInteractivelyPresent
