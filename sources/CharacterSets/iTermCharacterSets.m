...
// ============================================================================
// Bidi mirroring (BidiMirroring.txt, UBA rule L4)
// ============================================================================

// (code point, Bidi_Mirroring_Glyph counterpart) sorted by code point.
// Every pair in BidiMirroring.txt is in the BMP; the generator fails if a
// future Unicode version breaks that assumption.
static const uint16_t sBidiMirroringPairs[][2] = {
    {0x0028, 0x0029},
    {0x0029, 0x0028},
    {0x003C, 0x003E},
    {0x003E, 0x003C},
...
static const int sBidiMirroringPairCount = 428;

unichar iTermBidiMirroredCounterpart(unichar cp) {
    int lo = 0;
    int hi = sBidiMirroringPairCount - 1;
    while (lo <= hi) {
        const int mid = (lo + hi) / 2;
        const uint16_t key = sBidiMirroringPairs[mid][0];
        if (key == cp) {
            return sBidiMirroringPairs[mid][1];
        }
        if (key < cp) {
            lo = mid + 1;
        } else {
