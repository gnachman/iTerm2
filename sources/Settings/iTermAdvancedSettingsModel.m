...
DEFINE_BOOL(keepAwakeWhileAgentRunning, YES, SECTION_GENERAL @"Prevent system idle sleep while a Claude Code agent is running.\nWhen enabled, the Mac will not go to sleep due to inactivity while claude is in any session's process ancestry. User-initiated sleep (lid close, Apple menu → Sleep) is unaffected.");
DEFINE_BOOL(keepDisplayAwakeWhileAgentRunning, YES, SECTION_GENERAL @"Prevent display sleep while a Claude Code agent is running.\nWhen enabled, the display will not turn off or lock due to inactivity while claude is in any session's process ancestry. Has no effect unless "Prevent system idle sleep" is also on. User-initiated sleep is unaffected.");
