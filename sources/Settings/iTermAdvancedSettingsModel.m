...
DEFINE_BOOL(keepAwakeWhileAgentRunning, YES, SECTION_GENERAL @"Prevent system idle sleep while an AI coding agent is running.\nWhen enabled, the Mac will not go to sleep due to inactivity while claude, codex, opencode, cline, or pi is in any session's process ancestry. User-initiated sleep (lid close, Apple menu → Sleep) is unaffected.");
DEFINE_BOOL(keepDisplayAwakeWhileAgentRunning, YES, SECTION_GENERAL @"Prevent display sleep while an AI coding agent is running.\nWhen enabled, the display will not turn off or lock due to inactivity while an AI coding agent is running. Has no effect unless the system idle sleep setting above is also on. User-initiated sleep is unaffected.");
