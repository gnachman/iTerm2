...
DEFINE_BOOL(stripZeroWidthFormatCharactersOnPaste, NO, SECTION_TERMINAL @"Strip zero-width bidi/format characters when pasting?\nCharacters like the Zero-Width Non-Joiner (U+200C, the Persian half-space), ZWJ, LRM, and RLM are invisible but macOS classifies them as control characters, so line editors like zsh render them as a visible <200c> at the prompt. Turn this on to remove them from pasted text so the command line stays clean. Off by default because these characters are meaningful; enabling this changes the pasted text.");
...
DEFINE_BOOL(isolateLatinRunsInRTL, NO, SECTION_EXPERIMENTAL @"Treat Latin (English) words as left-to-right islands in right-to-left text?\nKeeps English words, their surrounding parentheses/brackets, and file paths in natural left-to-right order instead of following the Unicode bidi algorithm. This deviates from how other apps render the same text. Requires right-to-left text support.");
...
DEFINE_INT(rtlParagraphMinimumWords, 0, SECTION_EXPERIMENTAL @"Treat a line as right-to-left when it contains at least this many right-to-left words.\n0 disables this rule. When set, a line right-justifies if it contains at least this many Arabic/Hebrew/Persian words, in addition to the normal detection (first strong character, or a right-to-left majority). Setting it to 1 right-justifies every line that mentions any right-to-left word, including a shell prompt with a pasted Persian word. Requires “auto-detect paragraph writing direction”.");
