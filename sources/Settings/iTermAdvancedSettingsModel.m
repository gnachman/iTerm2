...
DEFINE_BOOL(twoRowTabBar, NO, SECTION_TABS @"Allow the tab bar to use two rows.\nWhen enabled, tabs spread over a second row — doubling the tab bar's height — once there are more of them than would fit on one row, giving each tab more room for its title instead of squeezing them all onto a single row. Until then the tab bar looks exactly as it does with this setting off, so a wide window may need many tabs before the second row appears. Only affects top/bottom tab bars; left/right bars are unchanged.");
...
DEFINE_BOOL(preventSleepOnBatteryToo, NO, SECTION_GENERAL @"Prevent system idle sleep on battery too, for profiles with “Prevent sleep” enabled?\nBy default the “Prevent sleep” profile setting only prevents idle sleep while connected to power. Enable this to also prevent idle sleep on battery.");
