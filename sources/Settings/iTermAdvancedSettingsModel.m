...
DEFINE_BOOL(keepAwakeWhileAgentRunning, YES, SECTION_GENERAL @"Prevent idle sleep while a Claude Code agent is running.\nWhen enabled, iTerm2 holds a power-management assertion whenever one or more sessions have claude in their foreground-job ancestry, preventing the Mac from sleeping due to inactivity. User-initiated sleep (e.g. closing the lid) is not affected.");
