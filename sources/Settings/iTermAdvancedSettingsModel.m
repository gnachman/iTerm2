...
DEFINE_INT_ENUM(keepDisplayAwakeWhileAgentRunning, 1, (@[ @"Off", @"AC power only", @"Always" ]), SECTION_GENERAL @"Prevent display sleep while an AI coding agent is running.\nAC power only (the default) prevents display sleep and screen lock only when the Mac is plugged in. Always also applies on battery. Has no effect if system idle sleep prevention is Off or would not fire (e.g. system is set to AC power only and the Mac is on battery). User-initiated sleep is unaffected.");
DEFINE_SETTABLE_STRING(extraAgentJobNames, ExtraAgentJobNames, @"", SECTION_GENERAL @"Additional AI agent process names to keep the Mac awake for.\nComma-separated list of process names (argv[0]) to add to the built-in list (claude, codex, opencode, cline, pi). Example: aider,cursor");
