...
- (void)awakeFromNib {
    ...
    menu.delegate = self;
    [menu addItem:[[NSMenuItem alloc] initWithTitle:ITLocalize(@"SnippetsEditingViewController_Menu_Duplicate", @"Duplicate", @"menu item title")
                                             action:@selector(duplicateSnippets:)
                                      keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:ITLocalize(@"COMMON_DELETE", @"Delete", @"menu item title")
                                             action:@selector(deleteSnippets:)
                                      keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:ITLocalize(@"SnippetsEditingViewController_Menu_AddAbove", @"Add Above", @"menu item title")
                                             action:@selector(addSnippetAbove:)
                                      keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:ITLocalize(@"SnippetsEditingViewController_Menu_AddBelow", @"Add Below", @"menu item title")
                                             action:@selector(addSnippetBelow:)
                                      keyEquivalent:@""]];
    [menu addItem:[[NSMenuItem alloc] initWithTitle:ITLocalize(@"COMMON_EDIT", @"Edit", @"menu item title")
...
- (void)defineControlsInContainer:(iTermPreferencesBaseViewController *)container
    ...
    [container addViewToSearchIndex:_tableView
                        displayName:ITLocalize(@"SnippetsEditingViewController_Facing_Snippets", @"Snippets", @"Text shown in defineControlsInContainer:: Snippets")
                            phrases:@[ @"Snippets" ]
...
- (void)importURL:(NSURL *)url {
    ...
    NSString *content = [NSString stringWithContentsOfFile:url.path
                                                  encoding:NSUTF8StringEncoding
                                                     error:&error];
    if (!content || error) {
        [iTermWarning showWarningWithTitle:[NSString stringWithFormat:ITLocalize(@"SnippetsEditingViewController_Alert_WhileLoading_FORMAT", @"While loading %1$@: %2$@", @"Alert title in importURL:"), url.path, error.localizedDescription]
                                   actions:@[ ITLocalize(@"COMMON_OK", @"OK", @"Action title in importURL:") ]
                                 accessory:nil
                                identifier:@"NoSyncImportSnippetsFailed"
                               silenceable:kiTermWarningTypePersistent
                                   heading:ITLocalize(@"SnippetsEditingViewController_AlertHeading_ImportFailed", @"Import Failed",@"Alert heading in importURL:(NSURL *)url")
    ...
    if (!root) {
        [iTermWarning showWarningWithTitle:[NSString stringWithFormat:ITLocalize(@"SnippetsEditingViewController_Alert_WhileParsing_FORMAT", @"While parsing %1$@: %2$@", @"Alert title in importURL:"), url.path, error.localizedDescription]
                                   actions:@[ ITLocalize(@"COMMON_OK", @"OK", @"Action title in importURL:") ]
                                 accessory:nil
                                identifier:@"NoSyncImportSnippetsFailed"
                               silenceable:kiTermWarningTypePersistent
...
- (void)showEncodingErrorForURL:(NSURL *)url {
    [iTermWarning showWarningWithTitle:[NSString stringWithFormat:ITLocalize(@"SnippetsEditingViewController_Alert_MalformedFileAt_FORMAT", @"Malformed file at %1$@", @"Alert title in showEncodingErrorForURL:"), url.path]
                               actions:@[ ITLocalize(@"COMMON_OK", @"OK", @"Action title in showEncodingErrorForURL:") ]
                             accessory:nil
                            identifier:@"NoSyncSnippetEncodingError"
                           silenceable:kiTermWarningTypePersistent
                               heading:ITLocalize(@"SnippetsEditingViewController_AlertHeading_ImportFailed", @"Import Failed",@"Alert heading in showEncodingErrorForURL:(NSURL *)url")
                                window:self.view.window];
}
...
- (void)exportToItem:(iTermSavePanelItem *)item {
    ...
    [json writeToSaveItem:item completionHandler:^(NSError *error) {
        if (error) {
            [iTermWarning showWarningWithTitle:[NSString stringWithFormat:ITLocalize(@"SnippetsEditingViewController_Alert_ErrorSavingTo_FORMAT", @"Error saving to %1$@: %2$@", @"Alert title in exportToItem:"),
                                   ...
                                   silenceable:kiTermWarningTypePersistent
                                       heading:ITLocalize(@"SnippetsEditingViewController_AlertHeading_ExportFailed", @"Export Failed",@"Alert heading in exportToItem:(iTermSavePanelItem *)item")
                                        window:self.view.window];
        } else {
