...
// Tag of the bell sound menu item that opens a file chooser instead of naming a sound.
// Every other item leaves its tag at 0.
static const NSInteger iTermBellSoundOtherTag = 1;
...
@implementation ProfilesTerminalPreferencesViewController {
    ...
    IBOutlet NSPopUpButton *_bellSound;
    IBOutlet NSTextField *_bellSoundLabel;
...
- (void)awakeFromNib {
    ...
    info = [self defineControl:_silenceBell
                           key:KEY_SILENCE_BELL
                   relatedView:nil
                          type:kPreferenceInfoTypeCheckbox];
    info.observer = ^() {
        // The sound popup dims itself through its shouldBeEnabled block, but its label is
        // not a control the preference machinery knows about.
        __strong __typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
    ...
    info = [self defineControl:_bellSound
                           key:KEY_BELL_SOUND
                   relatedView:_bellSoundLabel
                          type:kPreferenceInfoTypeStringPopup];
    info.onUpdate = ^BOOL {
        // The menu has to already hold the profile’s sound for the normal path to select
        // it, and a profile naming a file of its own gets an item made for it.
        [weakSelf rebuildBellSounds];
        return NO;
    };
    info.customSettingChangedHandler = ^(id sender) {
        [weakSelf bellSoundDidChange];
    };
    info.shouldBeEnabled = ^BOOL {
        return ![weakSelf boolForKey:KEY_SILENCE_BELL];
    };
    [info addShouldBeEnabledDependencyOnSetting:KEY_SILENCE_BELL
                                     controller:self];
...
- (void)rebuildBellSounds {
    ...
    NSMenuItem *systemAlert = [[NSMenuItem alloc] init];
    systemAlert.title = NSLocalizedStringWithDefaultValue(@"ProfilesTerminal.SystemAlertSound", nil, [NSBundle mainBundle], @"System Alert Sound", @"Name in the bell sound menu of the Mac’s own alert sound, which is what the bell plays by default");
    systemAlert.representedObject = @"";
    [menu addItem:systemAlert];
    [menu addItem:[NSMenuItem separatorItem]];

    for (NSString *name in [iTermBellSound installedSoundNames]) {
        NSMenuItem *item = [[NSMenuItem alloc] init];
        item.title = name;  // Localization unneeded: these are the installed sounds’ own names.
        item.representedObject = name;
        [menu addItem:item];
    }

    // A profile can name a file that is not in a Sounds folder, and can outlive the file
    // it names. Either way it gets an item of its own so the popup shows what the profile
    // actually says rather than silently reading as the default.
    if (currentValue.length > 0 && [_bellSound indexOfItemWithRepresentedObject:currentValue] < 0) {
        [menu addItem:[NSMenuItem separatorItem]];
        NSMenuItem *item = [[NSMenuItem alloc] init];
        item.title = [iTermBellSound displayNameForProfileValue:currentValue];
    ...
    other.title = NSLocalizedStringWithDefaultValue(@"ProfilesTerminal.OtherBellSound", nil, [NSBundle mainBundle], @"Other…", @"Menu item that opens a file chooser to pick an audio file for the terminal bell");
    other.tag = iTermBellSoundOtherTag;
    [menu addItem:other];
}

- (void)bellSoundDidChange {
    if (_bellSound.selectedItem.tag == iTermBellSoundOtherTag) {
        [self browseForBellSound];
        return;
    }
    [self setString:_bellSound.selectedItem.representedObject forKey:KEY_BELL_SOUND];
    [self previewBellSound];
}

- (void)browseForBellSound {
...
- (void)previewBellSound {
    // Play it so the sounds can be told apart without leaving Settings.
    [[iTermBellSoundPlayer sharedInstance] playProfileValue:[self stringForKey:KEY_BELL_SOUND]];
}
