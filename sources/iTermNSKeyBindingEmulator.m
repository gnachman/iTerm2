...
static NSDictionary *gRootDictionary;

@interface iTermNSKeyBindingEmulator ()

// The current subtree.
@property(nonatomic, assign) NSDictionary *currentDict;
...
+ (void)initialize {
    ...
    if ([paths count]) {
        ...
        gRootDictionary = [[self keyBindingDictionaryByNormalizingModifiersInKeys:theDict] retain];
    }
}
...
- (BOOL)handlesEvent:(NSEvent *)event extraEvents:(NSMutableArray *)extraEvents {
    ...
    NSArray *possibleKeys = [[self class] dictionaryKeysForEvent:event];
    if (possibleKeys.count == 0) {
        self.currentDict = gRootDictionary;
