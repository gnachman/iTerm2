...
extension PTYTextView {
    /// Extends URL search results across soft boundaries (like tmux pane dividers).
    ///
    /// When a URL is split across visual lines due to a soft boundary, the search only finds
    /// the first part. This method detects such cases and extends the result to include the
    /// continuation on subsequent lines by re-running the URL regex on concatenated text.
    @objc
    func extendURLSearchResultsAcrossSoftBoundaries(_ results: NSMutableArray) {
        guard let dataSource else { return }
        guard let regex = try? NSRegularExpression(pattern: iTermAdvancedSettingsModel.findUrlsRegex(),
                                                    options: []) else {
            return
        }
        ...
        let screenHeight = dataSource.height()

        for case let result as SearchResult in results {
            ...
            // Create extractor and find the logical window at the start coordinate
            let extractor = iTermTextExtractor(dataSource: dataSource)
            ...
            // Check if result ends at or near the right edge of the logical window
            if result.internalEndX < rightEdge {
                continue
            }

            // Result ends at the soft boundary - build located string and re-run regex
            // Limit search to screen height lines beyond the result end
            let maxY = min(numberOfLines, endY + screenHeight)
            if let (newEndX, newEndY) = findExtendedURLEnd(
                result: result,
                startY: startY,
                endY: endY,
    ...
    private func findExtendedURLEnd(
        result: SearchResult,
        startY: Int32,
        endY: Int32,
        ...
        let rightEdge = logicalWindow.location + logicalWindow.length - 1
        let dividerColumn = logicalWindow.location + logicalWindow.length  // Column where divider should be

        // Build a located string starting from the result's start position
        let locatedString = iTermLocatedString()

        // Add text from start position to end of first line (within logical window)
        appendLineContent(
            to: locatedString,
            extractor: extractor,
            y: startY,
        ...
        // Add subsequent lines while the divider continues
        var currentY = endY + 1
        while currentY < maxY {
            // Check if there's still a divider at the expected column
            let dividerCoord = VT100GridCoord(x: dividerColumn, y: currentY)
            if !extractor.character(atCoordIsColumnDivider: dividerCoord) {
                break
            }

            // Append this line's content within the logical window
            appendLineContent(
                to: locatedString,
                extractor: extractor,
                y: currentY,
        ...
        // Run the URL regex on the concatenated string
        ...
        // The match should start at position 0 since we started from the URL's beginning
        ...
        // Only extend if we actually found a longer match
    ...
    private func appendLineContent(
