...
extension iTermApplicationDelegate {
    ...
    func registerMenuTips() {
        ...
        let tips = [
            ...
            Tip(identifier: "Lock Location",
                text: String(localized: "MenuTip.LockLocation", defaultValue: "When a window’s location is locked, it returns to the display and position it had when you locked it if anything moves it. Window styles that fill or attach to the screen, such as Maximized, Centered, and the top, bottom, left, and right styles, keep their usual shape and are held to their display instead. If that display is disconnected, as happens when a laptop sleeps, the window waits and goes back when the display returns. Dragging the window unlocks it. Note that macOS puts a returning window on whatever Space its display is currently showing, which may not be the Space it started on.", comment: "Menu item help for the ‘Lock Location’ menu item")),
