...
class iTermSuppressedAlertsWindowController: NSWindowController {
    ...
    private init() {
        let panel = NSPanel(contentRect: NSRect(x: 0, y: 0, width: 640, height: 380),
                            ...
                            backing: .buffered,
                            defer: true)
        panel.title = String(localized: "SuppressedAlerts.WindowTitle", defaultValue: "Suppressed Alerts", comment: "Title of the suppressed-alerts panel")
    ...
    private func buildContentView() {
        ...
        let explanation = label(withText: String(localized: "SuppressedAlerts.Explanation", defaultValue: "These alerts are being answered automatically because you chose to remember your response. Select one and click Un-suppress to be asked again.", comment: "Explanatory text at the top of the suppressed-alerts panel"))
        ...
        whenColumn.title = String(localized: "SuppressedAlerts.WhenColumn", defaultValue: "Last Suppressed", comment: "Column header for when the alert was last suppressed")
        whenColumn.width = 130
        whenColumn.minWidth = 100

        let responseColumn = NSTableColumn(identifier: Column.response)
        responseColumn.title = String(localized: "SuppressedAlerts.ResponseColumn", defaultValue: "Automatic Response", comment: "Column header for the automatic response given to the alert")
        responseColumn.width = 150
        responseColumn.minWidth = 100

        let alertColumn = NSTableColumn(identifier: Column.alert)
        alertColumn.title = String(localized: "SuppressedAlerts.AlertColumn", defaultValue: "Alert", comment: "Column header for the alert message")
        ...
        let empty = label(withText: String(localized: "SuppressedAlerts.EmptyState", defaultValue: "No alerts are currently being suppressed.", comment: "Empty-state message when no alerts are suppressed"))
        ...
        // Bottom buttons.
        let doneButton = NSButton(title: String(localized: "General.Done", defaultValue: "Done", comment: "Done button"), target: self, action: #selector(done(_:)))
        ...
        let unsuppress = NSButton(title: String(localized: "SuppressedAlerts.Unsuppress", defaultValue: "Un-suppress", comment: "Button to un-suppress the selected alert"), target: self, action: #selector(unsuppress(_:)))
        ...
        let unsuppressAll = NSButton(title: String(localized: "SuppressedAlerts.UnsuppressAll", defaultValue: "Un-suppress All", comment: "Button to un-suppress all suppressed alerts"), target: self, action: #selector(unsuppressAll(_:)))
        ...
        // response.
        let checkbox = NSButton(checkboxWithTitle: String(localized: "SuppressedAlerts.ShowRememberedCheckbox", defaultValue: "Always show alerts with remembered selections", comment: "Checkbox to always show alerts that have a remembered response"),
                                target: self,
