...
- (int)_wrappedLineWithWrapWidth:(int)width
    ...
    *lineLength = location.length - offset;  // the length of the suffix of the raw line, beginning at the wrapped line we want
    ...
    if (bidiInfoPtr) {
        *bidiInfoPtr = [self subBidiInfo:md->bidi_display_info range:NSMakeRange(location.prev + offset, width) width:width];
    }
    if (lineOffset) {
        *lineOffset = offset;
    }
...
- (ScreenCharArray *)lastRawLine {
    ...
    return [[ScreenCharArray alloc] initWithLine:_characterBuffer.pointer + _startOffset
