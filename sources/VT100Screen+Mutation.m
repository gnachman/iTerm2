...
- (void)terminalAppendString:(NSString *)string {
    if (_state.collectInputForPrinting) {
        ...
        [_mutableState didAppendString:string];
    }
...
- (NSInteger)mutNumberOfTriggers {
    return [self numberOfTriggers];
}

- (NSArray<NSString *> *)mutTriggerNames {
    return [self triggerNames];
}

- (NSIndexSet *)mutEnabledTriggerIndexes {
    return [self enabledTriggerIndexes];
}
