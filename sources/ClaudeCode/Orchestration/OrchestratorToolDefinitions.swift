...
extension OrchestratorCommand {
    ...
    static var allToolDefinitions: [ToolDefinition] { [
        ...
        ToolDefinition(
            ...
            description: "Run a Code Review in one call. The session_guid must name the Code Review role's session. The session can be in either of two ready states: (a) the prompt overlay is up (pending_action mentions \u{201C}Code Review prompt overlay\u{201D}), in which case the overlay is populated and the review program launches; or (b) the Code Review session already ran and is idle at its chat prompt, in which case the session is reloaded (killed and relaunched) to run a fresh review. Either way the call auto-registers a watcher for the session reaching idle, so you'll receive a status_update when the review completes. The call errors only if the session isn't the Code Review role, its program is busy, or it can't be reloaded. Pick the prompt by name from the saved prompts (see the system message for the list) via prompt_name, OR pass custom_prompt with free-form text, OR leave both nil to use the user's default prompt. Prefer this over send_text + register_watch for code reviews.",
            inputSchema: object([
                ("session_guid", sessionGuidSchema),
