...
void StringToScreenChars(NSString *s,
    ...
    *rtlFound = NO;
    ...
    if (rtlFound && [s rangeOfCharacterFromSet:rtlSmellingCodePoints].location != NSNotFound) {
        *rtlFound = YES;
    }
...
BOOL AnnotateRightToLeftInScreenChars(screen_char_t *c, int len) {
    ...
    // We do not want to include trailing empty cells in the string because they are always LTR.
    int emptyCount = 0;
    while (emptyCount < len && c[len - emptyCount - 1].code == 0) {
        emptyCount += 1;
    }

    NSString *string = ScreenCharArrayToString(c, 0, len - emptyCount, &buffer, &deltas);
    NSIndexSet *rtlIndexes = [string rtlIndexes];
    if (rtlIndexes.count == 0) {
        // Fast path
        for (int i = 0; i < len; i++) {
            c[i].rtlStatus = RTLStatusLTR;
        }
    } else {
        for (int i = 0; i < len - emptyCount; i++) {
            const NSInteger stringIndex = UTF16OffsetFromCellOffset(i, deltas, len);
            c[i].rtlStatus = [rtlIndexes containsIndex:stringIndex] ? RTLStatusRTL : RTLStatusLTR;
        }
        for (int i = emptyCount; i < len; i++) {
            c[i].rtlStatus = RTLStatusLTR;
        }
    }
    free(deltas);
