...
BOOL IsCombiningMark(UTF32Char c) {
    ...
    if (!combiningMarks) {
        struct {
            ...
            // It's not quite clear what the definition of a combining character is in Unicode 8.0.
            // In Unicode 6.0, it was defined in chapter 3, definition 52, as characters
            // whose general category was Mc, Mn, or Me. The 8.0 spec doesn't say, and it's likely
            // the case that we oversimpify handling of combining characters. Looking at just the
            // canonical combining class misses all kinds of characters that combine. For now I'm
            // sticking with Mc+Mn+Me. That gets everything with combining class != 0 plus many
            // others. Also, note that some characters that combine with preceding characters don't
            // fit in this definition. So far, I've found the skin tone modifiers in U+1f3fb through
            // U+1f3ff, which I've manually added here as a bit of a hack.
            //
            // This is generated by the range.py script processing code points extracted from
            // UnicodeData.txt with this sql statement:
            //
            // select Number from UnicodeData where
            //     GeneralCategory='Mc' or GeneralCategory='Mn' or GeneralCategory='Me';
            //
            // You can import unicode data in sqlite3 like this:
            // create table UnicodeData (
            //    Number text,
            //    Name text,
            //    GeneralCategory text,
            //    CanonicalCombiningClass text,
            //    BidiClass text,
            //    DecompositionType text,
            //    DecompositionMapping text,
            //    NumericType text,
            //    NumericValue text,
            //    BidiMirrored text,
            //    Unicode1Name text,
            //    ISOComment text,
            //    SimpleUppercaseMapping text,
            //    SimpleLowercaseMapping text,
            //    SimpleTitlecaseMapping text);
            // .separator ";"
            // .import UnicodeData.csv UnicodeData
            ...
            { 0x825, 0x827 }, { 0x829, 0x82d }, { 0x859, 0x85b }, { 0x8e3, 0x903 },
            ...
            { 0xbd7, 0xbd7 }, { 0xc00, 0xc03 }, { 0xc3e, 0xc44 }, { 0xc46, 0xc48 },
            { 0xc4a, 0xc4d }, { 0xc55, 0xc56 }, { 0xc62, 0xc63 }, { 0xc81, 0xc83 },
            { 0xcbc, 0xcbc }, { 0xcbe, 0xcc4 }, { 0xcc6, 0xcc8 }, { 0xcca, 0xccd },
            { 0xcd5, 0xcd6 }, { 0xce2, 0xce3 }, { 0xd01, 0xd03 }, { 0xd3e, 0xd44 },
            ...
            { 0x1a55, 0x1a5e }, { 0x1a60, 0x1a7c }, { 0x1a7f, 0x1a7f }, { 0x1ab0, 0x1abe },
            { 0x1b00, 0x1b04 }, { 0x1b34, 0x1b44 }, { 0x1b6b, 0x1b73 }, { 0x1b80, 0x1b82 },
            { 0x1ba1, 0x1bad }, { 0x1be6, 0x1bf3 }, { 0x1c24, 0x1c37 }, { 0x1cd0, 0x1cd2 },
            { 0x1cd4, 0x1ce8 }, { 0x1ced, 0x1ced }, { 0x1cf2, 0x1cf4 }, { 0x1cf8, 0x1cf9 },
            ...
            { 0x10ae5, 0x10ae6 }, { 0x11000, 0x11002 }, { 0x11038, 0x11046 }, { 0x1107f, 0x11082 },
            { 0x110b0, 0x110ba }, { 0x11100, 0x11102 }, { 0x11127, 0x11134 }, { 0x11173, 0x11173 },
            { 0x11180, 0x11182 }, { 0x111b3, 0x111c0 }, { 0x111ca, 0x111cc }, { 0x1122c, 0x11237 },
            { 0x112df, 0x112ea }, { 0x11300, 0x11303 }, { 0x1133c, 0x1133c }, { 0x1133e, 0x11344 },
            { 0x11347, 0x11348 }, { 0x1134b, 0x1134d }, { 0x11357, 0x11357 }, { 0x11362, 0x11363 },
            { 0x11366, 0x1136c }, { 0x11370, 0x11374 }, { 0x114b0, 0x114c3 }, { 0x115af, 0x115b5 },
            { 0x115b8, 0x115c0 }, { 0x115dc, 0x115dd }, { 0x11630, 0x11640 }, { 0x116ab, 0x116b7 },
            { 0x1171d, 0x1172b }, { 0x16af0, 0x16af4 }, { 0x16b30, 0x16b36 }, { 0x16f51, 0x16f7e },
            ...
            { 0x1da9b, 0x1da9f }, { 0x1daa1, 0x1daaf }, { 0x1e8d0, 0x1e8d6 }, { 0xe0100, 0xe01ef },

            { 0x1f3fb, 0x1f3ff }  // Emoji skin tone modifiers, added by hand. This isn't quite right as these should combine only with Emoji.
        };

        NSMutableCharacterSet* temp = [[NSMutableCharacterSet alloc] init];
        for (int i = 0; i < sizeof(ranges) / sizeof(*ranges); i++) {
