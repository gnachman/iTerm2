...
class SyntaxHighlighter {
    ...
    func highlight(range rangeToModify: Range<Int>) {
        ...
        annotated.enumerateRoles { role, nsrange in
            ...
            print("Consider <<\(String(value.string.substringWithUTF16Range(shiftedRange)!))>> with role \(role)")
            switch role {
            case .command:
                print("Command in \(shiftedRange)")
                highlightCommand(shiftedRange)
            case .whitespace, .placeholder:
                break
            case .quoted:
                print("Quoted in \(shiftedRange)")
                highlightQuoted(shiftedRange)
            case .other:
                print("Argument in \(shiftedRange)")
    ...
    private static func check(fileChecker: FileChecker,
        ...
        fileChecker.checkIfFileExists(path: path) { [weak attributedString] exists in
            ...
            case .other:
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) { [weak attributedString] in
